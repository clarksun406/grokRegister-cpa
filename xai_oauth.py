...
CONSENT_NEXT_ACTION_ID = "4005315a1d7e426de592990bb54bb37471f39dd6d2"
...
def _playwright_click_first(page: Any, selectors: list[str]) -> bool:
    # 遍历所有 frame（含 iframe），consent 按钮可能在子 frame 里
...
def _playwright_submit_consent_action(page: Any) -> str:
    """Submit the consent Next.js Server Action and return its OAuth code."""
    current = urlparse(page.url)
    if "/oauth2/consent" not in current.path:
        return ""
    params = {key: values[-1] for key, values in parse_qs(current.query).items() if values}
    payload = json.dumps([{
        "action": "allow",
        "clientId": params.get("client_id", DEFAULT_CLIENT_ID),
        "redirectUri": params.get("redirect_uri", ""),
        "scope": params.get("scope", " ".join(DEFAULT_SCOPES)),
    ...
    result = page.evaluate(
        ...
        {"url": page.url, "body": payload, "action": CONSENT_NEXT_ACTION_ID},
    )
    ...
    for line in body.splitlines():
        ...
        try:
            ...
            continue
        if isinstance(data, dict) and data.get("success") is not False and data.get("code"):
...
def _playwright_drive_login(page: Any, email: str, password: str, sink: _CallbackState, deadline: float) -> None:
    ...
    consent_selectors = [
        ...
        'button:has-text("Continue")',
        # 中文 consent 页（accounts.x.ai/oauth2/consent）按钮为「允许」
        'button:has-text("允许")',
        'button[type="submit"]',
    ]
    ...
    while time.time() < deadline and not sink.event.is_set():
        try:
            ...
            if not filled_password and _playwright_fill_first(page, password_selectors, password):
                ...
                # consent 页：点 Allow/允许 提交授权。Allow 是 React 受控按钮，
                # consent 页：等 Allow 出现并 enabled，再点；JS click 兜底；
                # 检测页面文本变化（xAI consent 提交后 SPA 切到 device-code/跳转，URL 可能不变）。
                try:
                    loc = page.locator('button:has-text("Allow"), button:has-text("允许")')
                    if loc.count() > 0:
                        try:
                        ...
                        try:
                            ...
                            pass
                        # JS 兜底 click（绕过命中层/事件委托）
                        try:
                            page.evaluate(
                                "() => { const bs=[...document.querySelectorAll('button')];"
                                " const a=bs.find(b => /Allow|允许/.test((b.innerText||'').trim()));"
                                " if(a && !a.disabled){ a.click(); } }"
                            )
                        except Exception:
                            pass
                    # 检测页面文本/URL 变化，变化说明提交生效
                    try:
                        _txt = page.eval_on_selector_all(
                            "h1,h2,p",
                            "els => els.filter(b => b.offsetParent !== null).map(b => (b.innerText||'').trim()).filter(t => t).slice(0,3).join(' | ')",
                        )
                    except Exception:
                        _txt = ""
                    if not getattr(_drive_diag, "_last_txt", None):
                        _drive_diag._last_txt = _txt
                    if _txt and _txt != _drive_diag._last_txt:
                        _drive_diag._last_txt = _txt
                        print(f"  [diag-drive] consent 提交后文本变化: {_txt[:80]}", flush=True)
                except Exception:
                    pass
        except Exception:
            pass
...
def login_with_playwright(
    ...
    *,
    ...
    submit_consent_action: bool = True,
    auto_click_consent: bool = True,
) -> OAuthLoginResult:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # 监听 consent 提交的网络请求/响应，定位 Allow 点击后服务端是否拒绝
                _net_log: list = []
                def _on_resp(resp):
                    try:
                        if "consent" in resp.url or "authorize" in resp.url or "callback" in resp.url:
                            _net_log.append(f"{resp.status} {resp.request.method} {resp.url[:100]}")
                    except Exception:
                        pass
                page.on("response", _on_resp)
                print(f"  [diag] goto authorize: {auth_url[:90]}...", flush=True)
                try:
                    page.goto(auth_url, wait_until="domcontentloaded", timeout=int(min(timeout, 60) * 1000))
                except Exception as _ge:
                    print(f"  [diag] goto 失败: {_ge}", flush=True)
                try:
                    print(f"  [diag] goto 后 url={page.url} title={page.title()!r}", flush=True)
                except Exception as _e:
                    print(f"  [diag] goto 后无法读页面: {_e}", flush=True)
                if submit_consent_action and "/oauth2/consent" in page.url and not sink.event.is_set():
                    try:
                        direct_code = _playwright_submit_consent_action(page)
                        if direct_code:
                            print("  [oauth] consent Server Action 已返回 authorization code", flush=True)
                    except Exception as _consent_exc:
                        print(f"  [oauth] consent Server Action 失败，回退按钮点击: {_consent_exc}", flush=True)
                # If still on login form, fill credentials.
                if email and password and not direct_code and not sink.event.is_set():
                    _playwright_drive_login(page, email, password, sink, deadline)
                # Auto-click consent/authorize if present.
                _diag_logged = False
                _btn_logged = False
                while not direct_code and time.time() < deadline and not sink.event.is_set():
                    try:
                        if auto_click_consent:
                            for sel in (
                                'button:has-text("Authorize")',
                                'button:has-text("Allow")',
                                'button:has-text("Accept")',
                                ...
                                if loc.count() > 0 and loc.first.is_visible():
                                    ...
                                    break
                        # 打印一次页面上所有可见按钮文字，便于定位 consent 按钮没点中的原因
                        if not _btn_logged and (deadline - time.time()) < 150.0:
                            _btn_logged = True
                            try:
                                texts = page.eval_on_selector_all(
                                    "button",
                                    "els => els.filter(b => b.offsetParent !== null).map(b => b.innerText.trim()).filter(t => t)",
                                )
                                print(f"  [diag] 页面可见按钮: {texts}", flush=True)
                            except Exception as _e:
                                print(f"  [diag] 无法读取按钮: {_e}", flush=True)
                    except Exception:
                        pass
                    # 临近超时打印一次诊断（callback 没触发，看页面停在哪）
                    if not _diag_logged and (deadline - time.time()) < 15.0:
                        _diag_logged = True
                        try:
                            print(f"  [diag] callback 未触发，当前 url={page.url} title={page.title()!r}", flush=True)
                        except Exception as _e:
                            print(f"  [diag] 无法读取页面状态: {_e}", flush=True)
